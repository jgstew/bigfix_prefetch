...
def validate_prefetch(
    prefetch_test, sha256_required=False
    ...
    if "file_size" in prefetch_test:
        ...
        try:
            ...
            if str(err) != "'NoneType' object has no attribute 'group'":
                raise
            warnings.warn(
                "ERROR: prefetch is invalid, could not be parsed\n"
                + prefetch_test
                + "\n"
                + str(err)
    ...
    if int(parsed_bigfix_prefetch["file_size"]) <= 0:
        warnings.warn(
            "ERROR: prefetch size is invalid\n" + parsed_bigfix_prefetch["raw_prefetch"]
        )
        return False
    ...
    if "file_sha1" not in parsed_bigfix_prefetch:
        ...
        if (
            "prefetch_type" in parsed_bigfix_prefetch
